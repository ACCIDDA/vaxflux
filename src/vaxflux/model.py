__all__ = ("build_model", "strata_change_detection")
...
def strata_change_detection(
    ...
    Returns:
        A pandas DataFrame with the columns "param", "name", "test", "metric",
    ...
    if not np.all(
        trace_before.posterior.coords.get("strata").to_numpy()
        == trace_after.posterior.coords.get("strata").to_numpy()
    ):
        raise ValueError(
            "The 'strata' coordinates do not match between the two traces."
        )

    results = []
    for p in param:
        for s in trace_before.posterior.coords.get("strata").to_numpy().tolist():
            x = trace_before.posterior[f"{p}Strata"].sel(strata=s).to_numpy().flatten()
            y = trace_after.posterior[f"{p}Strata"].sel(strata=s).to_numpy().flatten()
